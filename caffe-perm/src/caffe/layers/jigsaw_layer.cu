...
__global__ void jigsaw_currand_init_kernel(curandState *state, const int seed)
{
...
__global__ void jigsaw_crop_kernel(const int n, curandState *state, const int top,
...
template <typename Dtype>
void JigsawLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
	...
	Blob<int>jigsaws(init_shape); this->generate_jigsaws(jigsaws, (*top[top.size()-1]), (*bottom[1]));
//	//DEBUG CODE
...
//			debug += boost::lexical_cast<std::string>(jigsaws.data_at(batch_idx, perm_idx, 0, 0));
//		}
//
...
//	debug_file.close();
	...
	const int* perm_gpu_data = jigsaws.gpu_data();
	...
	if(!isCurandInit){
		...
		jigsaw_currand_init_kernel<<<1, NBlocks>>>(devStates, time(NULL));
	...
	for(int top_idx = 0; top_idx < top.size()-1; top_idx++){
		...
		jigsaw_crop_kernel<Dtype> <<<NBlocks, NThreads>>>(
...
template <typename Dtype>
void JigsawLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
		const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
	return;
}

INSTANTIATE_LAYER_GPU_FUNCS(JigsawLayer);
