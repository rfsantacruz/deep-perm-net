...
__global__ void perm_matrix_currand_init_kernel(curandState *state, const int seed)
{
...
__global__ void perm_matrix_crop_kernel(const int n, curandState *state, const int top,
...
template <typename Dtype>
void PermMatrixLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
//	debug_file.close();
	...
	if(!isCurandInit){
		...
		perm_matrix_currand_init_kernel<<<1, NBlocks>>>(devStates, time(NULL));
	...
	for(int top_idx = 0; top_idx < top.size()-1; top_idx++){
		...
		perm_matrix_crop_kernel<Dtype> <<<NBlocks, NThreads>>>(
...
template <typename Dtype>
void PermMatrixLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
		const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
	return;
}

INSTANTIATE_LAYER_GPU_FUNCS(PermMatrixLayer);
