...
#include "caffe/layers/perm_matrix_layer.hpp"
...
void PermMatrixLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
	// LayerSetup() handles the number of dimensions;
	// bottom[0] supplies the data
	// bottom[1] supplies the original labels
	const PermMatrixParameter& param = this->layer_param_.perm_matrix_param();
	...
	if (param.perm_subset_size() > 0){
		vector<int> init_shape  = boost::assign::list_of(param.perm_subset_size())(pow(param.grid_size(),2));
...
template <typename Dtype>
void PermMatrixLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
	...
	tops_new_shape = boost::assign::list_of(bottom[0]->shape(0))(pow(param.grid_size(), 4));
	top[top.size() - 1]->Reshape(tops_new_shape);
}


template <typename Dtype>
void PermMatrixLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
	NOT_IMPLEMENTED;
}
...
template <typename Dtype>
void PermMatrixLayer<Dtype>::generate_perms(Blob<Dtype>& batch_perms, Blob<Dtype>& perm_matrix, Blob<Dtype>& target_perms) {
	...
	int perm_lenght = pow(param.grid_size(),2);
...
template <typename Dtype>
void PermMatrixLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
		const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
	NOT_IMPLEMENTED;
}


INSTANTIATE_CLASS(PermMatrixLayer);
REGISTER_LAYER_CLASS(PermMatrix);
