...
__global__ void fillin_currand_init_kernel(curandState *state, const int seed)
{
...
__global__ void fillin_crop_kernel(const int n, curandState *state, const int top,
...
template <typename Dtype>
void FillinLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
	...
	Blob<int>fillin(init_shape); this->generate_fillin(fillin, (*top[top.size()-1]));
//	//DEBUG CODE
...
//			debug += boost::lexical_cast<std::string>(fillin.data_at(batch_idx, perm_idx, 0, 0));
//		}
//
...
//	debug_file.close();
	...
	const int* perm_gpu_data = fillin.gpu_data();
	...
	if(!isCurandInit){
		...
		fillin_currand_init_kernel<<<1, NBlocks>>>(devStates, time(NULL));
	...
	for(int top_idx = 0; top_idx < top.size()-1; top_idx++){
		...
		fillin_crop_kernel<Dtype> <<<NBlocks, NThreads>>>(
...
template <typename Dtype>
void FillinLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
		const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
	return;
}

INSTANTIATE_LAYER_GPU_FUNCS(FillinLayer);
